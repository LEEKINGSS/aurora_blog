<template>
  ...
  <AuroraNavigator />
<!--  <Dia v-if="!isMobile && i18n.global.locale === 'cn'" />-->
<!--  <Dia v-else-if="!isMobile"/>-->
  <Dia v-if="!isMobile"/>
  <UserCenter />
  <teleport to="head">
    <title>{{ title }}</title>
...
export default defineComponent({
  ...
  setup() {
    const appStore = useAppStore() // 获取博客主题
    ...
    // 浏览器大小法师变化时使用
    const resizeHander = () => {
      // 获取 <body> 元素的边界框信息。getBoundingClientRect() 方法返回一个 DOMRect 对象，包含了元素的位置和尺寸信息
    ...
    return {
      language: ref(cookies.get('locale') ? String(cookies.get('locale')) : 'cn'),
