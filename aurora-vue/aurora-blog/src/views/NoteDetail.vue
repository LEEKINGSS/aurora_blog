    ...
    <div class="main-grid">
      <div class="post-header">
        <span class="post-labels">
          ...
          <b v-else-if="!loading && note.collectionName">
            <span>{{ note.collectionName }}</span>
          </b>
          <b v-else>{{ t('settings.default-category') }}</b>
          <ul>
            <ob-skeleton v-if="loading" :count="2" tag="li" height="16px" width="35px" class="mr-2" />
            <template v-else-if="!loading && note.tags && note.tags.length > 0">
        ...
        <h1 v-if="note.noteTitle" class="post-title text-white">
          {{ note.noteTitle }}
        </h1>
        <ob-skeleton
          v-else
        ...
        <div class="flex flex-row items-center justify-start mt-8 mb-4">
          <div class="post-footer" v-if="note.author">
            <img
              class="hover:opacity-50 cursor-pointer"
              v-lazy="note.author.avatar || ''"
              alt="author avatar"
              @click="handleAuthorClick(note.author.website)" />
    ...
    <div class="main-grid">
      <div>
        ...
        <div class="flex flex-col lg:flex-row justify-start items-end my-8 my-gap">
          <div class="w-full h-full self-stretch mr-0 lg:mr-4" v-if="preNoteCard">
            <SubTitle title="settings.paginator.pre" icon="arrow-left-circle" />
            <NoteDetailCard class="pre-and-next-article" :data="preNoteCard" />
          </div>
          <div class="w-full h-full self-stretch mt-0" v-if="nextNoteCard">
            <SubTitle title="settings.paginator.next" :side="!isMobile ? 'right' : 'left'" icon="arrow-right-circle" />
            <NoteDetailCard class="pre-and-next-article" :data="nextNoteCard" />
          </div>
        </div>
        <Comment />
      </div>
...
import { NoteDetailCard } from '@/components/NoteCard'
...
export default defineComponent({
  name: 'NoteSDetail',
  components: { NoteDetailCard, ObSkeleton, Sidebar, Comment, SubTitle, ArticleCard, Profile, Sticky, Navigator },
  setup() {
    ...
    commentStore.type = 1
    onMounted(() => {
      reactiveData.noteId = route.params.noteId
      toPageTop()
      fetchNote()
      fetchComments()
      appStore.loadStyle()
    ...
    onBeforeRouteUpdate((to) => {
      ...
      reactiveData.images = []
      reactiveData.preNoteCard = ''
      reactiveData.nextNoteCard = ''
      reactiveData.noteId = to.params.noteId
      pageInfo.current = 1
    ...
    emitter.on('noteFetchComment', () => {
      pageInfo.current = 1
      reactiveData.isReload = true
      fetchComments()
    })
    emitter.on('noteFetchReplies', (index) => {
      fetchReplies(index)
    })

    emitter.on('noteLoadMore', () => {
      fetchComments()
    })
    ...
    const initTocbot = () => {
      ...
      const imgs = noteRef.value.getElementsByTagName('img')
    ...
    const fetchNote = () => {
      loading.value = true
      api.getNoteById(reactiveData.noteId).then(({ data }) => {
        console.log(data)
        if (data.code === 52003) {
          proxy.$notify({
            title: 'Error',
        ...
        commonStore.setHeaderImage(data.data.noteCover)
        new Promise((resolve) => {
          data.data.noteContent = markdownToHtml(data.data.noteContent)
          resolve(data.data)
        }).then((note: any) => {
          reactiveData.note = note
          reactiveData.wordNum = Math.round(deleteHTMLTag(note.noteContent).length / 100) / 10 + 'k'
          reactiveData.readTime = Math.round(deleteHTMLTag(note.noteContent).length / 400) + 'mins'
          loading.value = false
          nextTick(() => {
            Prism.highlightAll()
            initTocbot()
        ...
        new Promise((resolve) => {
          data.data.preNoteCard.noteContent = markdownToHtml(data.data.preNoteCard.noteContent)
            .replace(/<\/?[^>]*>/g, '')
            .replace(/[|]*\n/, '')
            .replace(/&npsp;/gi, '')
          resolve(data.data.preNoteCard)
        }).then((preNoteCard: any) => {
          reactiveData.preNoteCard = preNoteCard
        })
        new Promise((resolve) => {
          data.data.nextNoteCard.noteContent = markdownToHtml(data.data.nextNoteCard.noteContent)
            .replace(/<\/?[^>]*>/g, '')
            .replace(/[|]*\n/, '')
            .replace(/&npsp;/gi, '')
          resolve(data.data.nextNoteCard)
        }).then((nextNoteCard) => {
          reactiveData.nextNoteCard = nextNoteCard
        })
      })
    }
    const fetchComments = () => {
